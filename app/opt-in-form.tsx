...
export default function OptInForm() {
  const [form, setForm] = useState<OptInInput>(empty);
    ...
    return (
      <div className="rounded-2xl bg-forest px-8 py-12 text-center text-cream">
        ...
        <p className="mx-auto max-w-md text-cream/80">
          You're on the Fahrenheit One founders list. Watch your inbox — we'll share
          the story, invite you to events, and give you first access when founding
          memberships open.
        </p>
      </div>
    );
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      {error && (
        <p className="sm:col-span-2 text-sm text-terra">{error}</p>
      )}
      ...
      <div className="sm:col-span-2 mt-2">
        ...
        >
          {pending ? "Joining…" : "Join the Founders List"}
        </button>
